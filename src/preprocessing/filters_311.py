...
assigns each surviving row to an issue_group.
...
# Issue-group mapping: every included service_name maps to exactly one group.
...
# Ordered list of all 9 issue groups.
...
def filter_311(
    ...
    Steps applied in order:
      ...
      7. Assign issue_group to every surviving row.
    ...
    Returns:
        Filtered DataFrame with a new `issue_group` column.
        Index is reset.
    """
    df = df.copy()
    ...
    # Assign issue_group
    df["issue_group"] = df["service_name"].map(_SERVICE_NAME_TO_GROUP)

    n_missing_group = df["issue_group"].isna().sum()
    if n_missing_group > 0:
        unmapped = df.loc[df["issue_group"].isna(), "service_name"].unique().tolist()
        raise ValueError(
            f"filter_311: {n_missing_group} rows could not be assigned an "
            f"issue_group. Unmapped service_name values: {unmapped}"
        )
